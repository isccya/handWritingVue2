...
> 用队列缓存一次事件循环中需要执行的函数,通过一个变量控制.异步任务是通过优雅降级的方式,先采用promise.then 最后再采用setTimeout,是对js事件循环的应用.
...
> 父组件创建虚拟节点时候,创建子组件虚拟节点时候,会从父组件选项中拿到子组件的构造函数,并放在子组件虚拟节点上.在创建真实节点时候,会调用子组件虚拟节点的构造函数方法返回子组件真实节点,再进行渲染. 
